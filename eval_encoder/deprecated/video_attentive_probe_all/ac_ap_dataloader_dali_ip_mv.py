            ...
            if self.enable_res_zero_mask:
                ...
                try:
                    ...
                    def _ensure_y(arr):
                        ...
                        # HevcFeatureReader yields Y or (Y,U,V); we only need Y here
                    ...
                    # tA = _t.time()
                    # Sparse sampling path (single reader)  —— MV + 残差融合版
                            ...
                            if cur_idx in wanted:
                                ...
                                if pos in I_pos_set:
                                    ...
                                    mv_norm, _ = _mv_energy_norm(
                                        mvx_hw, mvy_hw, H0, W0,
                                        mv_unit_div=mv_unit_div,   # e.g., 4.0
                                        pct=mv_pct                 # e.g., 95.0
                                    )
                                    res_norm, _ = _residual_energy_norm(
                                        Y_res, pct=res_pct          # e.g., 95.0
                                    )

                                    # 4) 融合（weighted/sum/max/geomean 均可，默认 weighted）
                                    fused = _fuse_energy(
                                        mv_norm, res_norm,
                                        mode=fuse_mode,            # e.g., "weighted"
                                        w_mv=w_mv, w_res=w_res     # e.g., 1.0, 1.0
                                    )  # float32, H0×W0 in [0,1]
