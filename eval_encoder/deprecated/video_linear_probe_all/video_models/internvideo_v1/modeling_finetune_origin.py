...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        # print('input feature:', x.shape) #(16, 3, 16, 224, 224)
        x = self.patch_embed(x)
        B, _, _ = x.size()

        cls_tokens = self.cls_token.expand(self.batch_size, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
    ...
    def forward_check_variance(self, x):
        # print('input check:', x.shape) #(16, 3, 16, 224, 224)
        x = self.patch_embed(x)
        B, _, _ = x.size()
    ...
    def forward_features_attentive_probe(self, x): #(16,3,16,224,224)
        # print('input:', x.shape)
        y = self.forward_check_variance(x)
